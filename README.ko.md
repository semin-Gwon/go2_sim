...
- 시뮬레이션 환경용 기본 경량 USD scene 파일 포함.
...
| `assets/envs/room_env.usd` | GitHub에 포함한 기본 경량 Isaac Sim scene 파일입니다. |
...
GitHub에는 기본 경량 USD 환경 파일인 `assets/envs/room_env.usd`만 포함합니다. 다른 실험용 environment 파일과 apartment scene 생성 스크립트는 `local_archive/` 아래에 로컬 전용으로 보관하고 GitHub에서는 제외했습니다.
